...
class OboOptionalAttrs(object):
    ...
    attributes = {'def', 'defn', 'synonym', 'relationship', 'xref', 'subset', 'comment'}
    ...
    def get_optional_attrs(optional_attrs):
        ...
        """
        attrs_opt = {'def', 'defn', 'synonym', 'relationship', 'xref', 'subset', 'comment'}
        ...
        if isinstance(optional_attrs, str) and optional_attrs in attrs_opt:
            ...
            opts = {getnm(f) for f in optional_attrs if f in attrs_opt}
        if opts:
            return opts
